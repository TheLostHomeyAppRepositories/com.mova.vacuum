...
class MovaCloudClient {
  ...
  _modeStyleFor(did) {
    const model = this._models.get(String(did));
  ...
  _logicalCleaningMode(did, raw) {
    const n = Number(raw);
    if (!Number.isFinite(n) || this._modeStyleFor(did) !== 's70') {
      return raw;
    }
    if (n === CLEANING_MODE.VACUUM) {
      return CLEANING_MODE.VACUUM_AND_MOP;
    }
    if (n === CLEANING_MODE.VACUUM_AND_MOP) {
      return CLEANING_MODE.VACUUM;
    }
    return raw;
  }

  _cleaningModeValue(kind, did) {
    const table = this._modeStyleFor(did) === 's70' ? CLEANING_MODE_S70 : CLEANING_MODE;
    if (kind === 'mop') {
      return table.MOP;
    }
    if (kind === 'vac_mop') {
  ...
  async startVacuum(did) {
    await this.setCleaningMode(did, this._cleaningModeValue('vacuum', did));
  ...
  async startMop(did, mode = 'mop') {
    const cleaningMode = this._cleaningModeValue(mode === 'vac_mop' ? 'vac_mop' : 'mop', did);
